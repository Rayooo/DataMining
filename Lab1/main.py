...
# 100k数据集读取方法
data_folder = os.path.join(os.path.expanduser("~"), "Documents/DataMining/DataMining/Lab1", "ml-100k")
...
# print(all_ratings[:15])
#
# print(ratings[:15])
#
# print(favorable_ratings[:15])
    ...
    for user, reviews in favorable_reviews_by_users.items():
        ...
        for itemset in k_1_itemsets:
            if itemset.issubset(reviews):  # 如果是，表明用户已经为子集中的电影打过分了
                # 接下来，就要遍历用户打过分却没有出现在项集reviews中的电影了,因为这样可以用它来生成超集，更新该项集的计数
                for other_reviewed_movie in reviews - itemset:
                    # other_reviewed_movie 用户打过分，但还不在频繁项集中
...
# 创建一个字典，存不同长度的频繁项集  数据格式:  频繁项集长度   对应的频繁项集
...
max_length = 20  # 定义要找的频繁集的最大长度
for k in range(2, max_length):  # 循环频繁集长度从2到 max_length
...
###########################################################
# 以上Apriori算法结束后，得到了一系列的频繁项集，但它还不是关联规则。
# 频繁项集是一组达到最小支持度的项目，而关联规则是由前提和结论组成
# 从频繁项集中抽取关联规则，把其中几部电影作为前提，另一部电影作为结论组成规则:
# 如果用户喜欢xx,yy,zz,那么他们也会喜欢ttt
# 遍历频繁项集，为每个项集生成规则
candidate_rules = []
...
for user, reviews in test_favorable_reviews_by_users.items():
    for candidate_rule in candidate_rules:  # 循环所有的规则
        premise, conclusion = candidate_rule
        # 判断前提是否是 reviews中的一个子集， 并且结论也在 reviews中，说明这条规则应验，否则不适用
        if premise.issubset(reviews):
            if conclusion in reviews:
