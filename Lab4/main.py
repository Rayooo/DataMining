...
image = create_captcha("GENE", shear=1)
...
# 共有26个类别，每个类别（字母）用从0到25之间的一个整数表示。用多个神经元有多个输出，每个输出值在0到1之间。
# 因此，对类别使用一位有效码编码方法，每条数据就能得到26个输出。如果结果像某字母，使用近似于1的值；如果不像，就用近似于0的值。
from sklearn.preprocessing import OneHotEncoder

onehot = OneHotEncoder()
y = onehot.fit_transform(targets.reshape(targets.shape[0], 1))

# 库不支持稀疏矩阵，因此，需要将稀疏矩阵转换为密集矩阵。
y = y.todense()
